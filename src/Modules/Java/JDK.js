...
function move(file) {
  return new Promise((resolve, reject) => {
    const newFile = path.join(javaFolder, file.split(path.sep).slice(-1)[0]);
 ...
 *   })
 */
export function install(version = 8, options = {}) {
  const {
  ...
  let url =
    'https://api.adoptopenjdk.net/v2/info/releases/openjdk' + version + '?';
